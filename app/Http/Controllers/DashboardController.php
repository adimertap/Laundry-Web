...
class DashboardController extends Controller
{
    public function index()
    {
        try {
            ...
            $pegawai_laundry_today_total = TransaksiLaundry::where('id_pegawai', Auth::user()->id)
            ->where('tanggal_transaksi', Carbon::now()->format('Y-m-d'))->sum('total');

            // BARIS 2
            $pegawai_count_money_today = Transaksi::where('id_pegawai', Auth::user()->id)
            ->where('tanggal_transaksi', Carbon::now()->format('Y-m-d'))->count();
            $pegawai_count_laundry_today =  TransaksiLaundry::where('id_pegawai', Auth::user()->id)
            ->where('tanggal_transaksi', Carbon::now()->format('Y-m-d'))->count();
            
            $pegawai_sum_money_bulan = Transaksi::where('id_pegawai', Auth::user()->id)->whereMonth('tanggal_transaksi', $month)->sum('total');
            $pegawai_sum_laundry_bulan = TransaksiLaundry::where('id_pegawai', Auth::user()->id)->whereMonth('tanggal_transaksi', $month)->sum('total');
            ...
            $transaksi_pegawai_laundry = TransaksiLaundry::with('detail')->where('id_pegawai', Auth::user()->id)
            ->where('tanggal_transaksi', Carbon::now()->format('Y-m-d'))
            ->orderBy('created_at','DESC')
            ->take(5)->get();
            ...
            return view('pages.dashboard.dashboard', compact(
                ...
                'pegawai_laundry_today_total',
                'pegawai_count_money_today',
                'pegawai_count_laundry_today',
                'pegawai_sum_money_bulan',
                'pegawai_sum_laundry_bulan',
                'transaksi_pegawai_money',
                'transaksi_pegawai_laundry'
            ));
        } catch (\Throwable $th) {
            dd($th);
        }
